...
interface DrawGuessLobbyProps {
  onCreateGame: (playerName: string) => void;
  onJoinGame: (playerName: string, gameCode: string) => void;
  gameCode?: string;
}

export default function DrawGuessLobby({ onCreateGame, onJoinGame, gameCode }: DrawGuessLobbyProps) {
  ...
  const shareUrl = gameCode ? `${window.location.origin}/games/draw-guess?code=${gameCode}` : "";
    ...
    return (
      <div className="flex flex-col items-center gap-6">
        ...
        <div className="text-center mb-2">
          <h2 className="font-black text-3xl uppercase tracking-tight mb-2">Draw & Guess</h2>
          <p className="text-sm text-gray-500 max-w-xs">One draws, the other guesses. Real-time canvas, no words allowed!</p>
        </div>
        ...
        {!connected && (
          <p className="text-xs text-red-500 text-center max-w-xs">
            Make sure you ran <code className="bg-red-100 px-1 rounded">npm run dev</code> (not <code className="bg-red-100 px-1 rounded">next dev</code>) to start the game server.
          </p>
        )}
      </div>
    );
